...
export function renderArticle({ config, article, qa, monetization, date, slug, visuals, commonsPhoto }) {
  ...
  const sections = qa.revisedSections.map(sectionHtml);
  if (commonsPhoto && sections.length > 1) sections.splice(1, 0, commonsPhotoFigure(commonsPhoto));
  if (visuals?.summary && sections.length > 3) {
    sections.splice(3, 0, visualFigure(visuals.summary, `${qa.revisedTitle} 핵심 요약 인포그래픽`, '본문의 검증된 핵심 내용을 자동으로 시각화한 요약 자료입니다.'));
  }
