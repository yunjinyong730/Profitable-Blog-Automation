...
function affiliateHtml(article, monetization) {
  ...
  return `<aside class="notice"><strong>관련 도구</strong><ul>${matches.map((link) => `<li><a href="${esc(safeUrl(link.url))}" rel="sponsored nofollow noopener">${esc(link.label || link.url)}</a></li>`).join('')}</ul><small>${esc(monetization.affiliateDisclosure)}</small></aside>`;
}
